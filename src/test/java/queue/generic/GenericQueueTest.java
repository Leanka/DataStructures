...
public class GenericQueueTest {
    ...
    public void testDequeueMultipleElements(){
        ...
        Integer expectedElement = 9;

        enqueueAndDequeueElements(numberOfElementsToAdd, numberOfElementsToRemove);
        enqueueAndDequeueElements(numberOfElementsToAdd, numberOfElementsToRemove);

        assertEquals(expectedElement, queue.peek());
    }

    @Test
    public void testIfDequeueReturnsFirstElement(){
        int numberOfRemovedElements = 1;
        Integer expectedElement = initialCapacity - numberOfRemovedElements;

        addElementsToQueue(initialCapacity);

        assertEquals(expectedElement, queue.dequeue());
    }
    ...
    private void enqueueAndDequeueElements(int countOfElementsToAdd, int countOfElementsToPop){
        addElementsToQueue(countOfElementsToAdd);
        popElementsFromQueue(countOfElementsToPop);
    }
